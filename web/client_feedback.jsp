    ...
    <body>
      ...
      %>
        ...
        <section class="container"  style="margin-top: 150px" >
            <table class="table">
                <tr><th>Cake name</th><th>Unit price</th><th>Quantity</th><th>Deliver date</th><th>delivery feedback</th><th>Comment</th></tr>
            <%
              if(request.getSession().getAttribute("user")!=null){
                ...
                for(InvoiceItem iitem:ini){
                    ...
                    if(dd!=null){
                    %>
                <tr><td><%=dd.getInvoiceItem().getCakestore().getCake().getName() %></td><td><%=dd.getInvoiceItem().getCakestore().getUnitPrice() %></td><td><%=dd.getInvoiceItem().getQty() %></td><td><%=dd.getDate() %></td><td><select id="rate" style="height: 30px"><option value="1">very good</option><option value="2">good</option><option value="3">fair</option><option value="4">Bad</option><option value="5">very Bad</option></select></td><td><input type="text" id="comment" style="height: 30px"></td><td><button class="btn btn-color" onclick="save(<%= dd.getInvoiceItem().getIdinvoiceItem() %>)">Add delivery feedback</button></td></tr>
                   <%
                    }
                }
                }
                ...
                request.onreadystatechange=function (){
                    if(request.readyState==4){
                        if(request.status===200){
                         ...
                         window.location="client_feedback.jsp";
