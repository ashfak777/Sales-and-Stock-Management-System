...
public class advancedsearch extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            Criteria c1 = ses.createCriteria(Category.class);
            c1.add(Restrictions.eq("categoryName", category));
            Category ctg = (Category) c1.uniqueResult();
            Criteria c2 = ses.createCriteria(CategoryHasCake.class);
            c2.add(Restrictions.eq("category", ctg));
            List<CategoryHasCake> chb1 = c2.list();

            int rcount = c2.list().size();
            int pages = 0;
            if (rcount % 15 == 0) {
                pages = rcount / 15;
            } else {
            ...
            c2.setFirstResult(Integer.parseInt(fr));
            c2.setMaxResults(15);
            Criteria cr2 = ses.createCriteria(userregistration.class);
            String content = "<tr>";

            for (CategoryHasCake chb2 : chb1) {
                Criteria cr = ses.createCriteria(Cake.class);
                cr.add(Restrictions.like("name", title, MatchMode.START));
                Criteria c = ses.createCriteria(Seller.class);
                c.add(Restrictions.eq("fname", sellername));
                Seller a = (Seller) c.uniqueResult();
                if (!sellername.equals(null)) {
                    cr.add(Restrictions.eq("seller", a));
                }
                //
                ...
                for (Cake b : cake) {
                    if (b.getIdcake() == (chb2.getCake().getIdcake())) {
                        ...
                        if (b.getUserRegistration().getStatus().equals("active")) {
                            ...
                            if ((!from.equals("")) && (!to.equals(""))) {
                                cr1.add(Restrictions.between("unitPrice", Double.parseDouble(from), Double.parseDouble(to)));
                            }
                            //  cr1.add(Restrictions.gt("unitPrice",Double.parseDouble(to)));
                            //  cr1.add(Restrictions.lt("unitPrice", to));
